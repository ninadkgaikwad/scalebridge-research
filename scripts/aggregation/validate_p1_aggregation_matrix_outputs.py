...
def audit(
    *,
    ...
    lines += ["", "3. COVERAGE", "-" * 80]
    building_counts = Counter(str(row.get("building_type", "")) for row in case_rows)
    weather_counts = Counter(str(row.get("weather_location", "")) for row in case_rows)
    aggregation_counts = Counter(str(row.get("plan_aggregation_id", "")) for row in case_rows)
    weight_counts = Counter(str(row.get("plan_weight_mode", "")) for row in case_rows)
    lines += [
...
def inspect_parquet_samples(run_roots: list[Path], sample_count: int) -> tuple[list[str], list[str], list[str]]:
    ...
    for path in sampled:
        ...
        missing = [col for col in REQUIRED_NODE_OUTPUT_COLUMNS if col not in names]
        for col in missing:
