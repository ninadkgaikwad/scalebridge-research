...
def test_ebp_source_uses_neuromancer_rk4_torch_solve_and_no_explicit_inverse_or_torchdiffeq():
    method_source = (Path(__file__).parents[1] / "src" / "pinode_epsr" / "methods" / "ebp_pinode.py").read_text(encoding="utf-8")
    projection_source = (Path(__file__).parents[1] / "src" / "pinode_epsr" / "physics" / "energy_projection.py").read_text(encoding="utf-8")
    source = method_source + "\n" + projection_source
    assert "rk4_interval(" in method_source
    assert "weighted_energy_projection" in method_source
    assert "torch.linalg.solve" in projection_source
    assert "return g_P" in method_source
