...
def build_single_run_command(
    *,
    ...
    ml_input_lags: list[int],
    ml_target_horizons: list[int],
    ...
    runner_configuration = {
        ...
        "ml_input_lags": list(ml_input_lags),
        "ml_target_horizons": list(ml_target_horizons),
    ...
    for lag in ml_input_lags:
        cmd.extend(["--ml-input-lag", str(lag)])
    for horizon in ml_target_horizons:
        cmd.extend(["--ml-target-horizon", str(horizon)])
...
def run_campaign(
    *,
    ...
    summary = {
        ...
        "ml_dataset_count": sum(1 for r in dataset_rows if str(r.get("silo")) == "ml_sciml"),
        "opt_bayes_dataset_count": sum(1 for r in dataset_rows if str(r.get("silo")) == "opt_bayes"),
        "ind_dataset_count": sum(1 for r in dataset_rows if str(r.get("mode")) == "independent"),
        "dep1_dataset_count": sum(1 for r in dataset_rows if str(r.get("mode")) == "dependent1"),
        "dep2_dataset_count": sum(1 for r in dataset_rows if str(r.get("mode")) == "dependent2"),
        "runtime_seconds": time.perf_counter() - started,
        "intermediate_time_series_persisted": False,
        "campaign_run_root": str(campaign_run_root),
        "mlflow_enabled": bool(mlflow),
    ...
    tracker.log_campaign_artifacts(campaign_run_root)
    tracker.finish(failed=(summary["status"] == "failed"))
    return summary
