...
def availability_from_canonical_table(
    ...
    table: pd.DataFrame,
    *,
    qac_available: bool | None = None,
) -> ZoneSignalAvailability:
    """Derive D6 usable signal availability from the validated D4 table.

    ``qac_available`` is supplied by the D4 Phase-C applicability/classification
    record during production assembly. ``None`` preserves the historical strict
    behavior used by direct callers: QAC must exist and be fully populated.
    """

    required = {"timestamp", STATE_SIGNAL, "outdoor_temperature"}
    ...
    if qac_available is None:
        qac_available = True
    if qac_available:
        if CONTROL_SIGNAL not in table.columns:
            raise D6ContractError(
                f"Canonical zone table is missing required QAC column for "
                f"{aggregate_zone_id!r}"
            )
    ...
    return ZoneSignalAvailability(
        ...
        qac_available=bool(qac_available),
    )
...
def assemble_zone_in_memory(
    *,
    ...
    qac_record = next(
        (record for record in result.signal_records if record.signal_name == CONTROL_SIGNAL),
        None,
    )
    if qac_record is None:
        raise D6ContractError(
            f"Canonical assembly did not produce a QAC applicability record for "
            f"{aggregate_zone_id!r}"
        )
    qac_available = not qac_record.nullable

    availability = availability_from_canonical_table(
        aggregate_zone_id,
        result.table,
        qac_available=qac_available,
    )
