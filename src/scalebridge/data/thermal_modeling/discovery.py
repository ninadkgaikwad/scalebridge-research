...
def _storage_token(value: str) -> str:
    """Normalize a logical zone ID to the stable Phase B storage-token form."""

    import re

    return re.sub(r"[^A-Za-z0-9]+", "_", str(value)).strip("_").casefold()


def _resolve_aggregation_zone_root(
    aggregation_run: AggregationRunRef,
    aggregate_zone_id: str,
) -> Path:
    """Resolve a Phase B zone directory without changing its logical zone ID.

    Phase B may store filesystem-safe directory names (for example
    ``G_Corridor``) while lineage retains the logical aggregate-zone ID
    (for example ``G Corridor``). Exact path identity remains authoritative
    when available. Otherwise a unique storage-token-equivalent directory is
    accepted; zero or multiple matches remain hard discovery failures.
    """

    zones_root = _required_dir(
        aggregation_run.run_root / "zones",
        "aggregation zones root",
    )
    exact = zones_root / aggregate_zone_id
    if exact.is_dir():
        return exact.resolve()

    wanted = _storage_token(aggregate_zone_id)
    matches = [
        path.resolve()
        for path in zones_root.iterdir()
        if path.is_dir() and _storage_token(path.name) == wanted
    ]
    if len(matches) == 1:
        return matches[0]
    if len(matches) > 1:
        raise PhaseDDiscoveryError(
            "Aggregation zone storage-name resolution is ambiguous for "
            f"{aggregate_zone_id!r}: {matches}"
        )
    raise PhaseDDiscoveryError(
        "Missing required aggregation zone root for logical zone "
        f"{aggregate_zone_id!r} under {zones_root}"
    )
...
def resolve_aggregation_zone(
    *,
    ...
    zone_root = _resolve_aggregation_zone_root(
        aggregation_run,
        aggregate_zone_id,
    )
...
def _resolve_zone_leaf(
    *,
    ...
    preferred_parents: list[Path] = []

    if aggregation_run_id:
        preferred_parents.append(
            run_root
            / "cases"
            / case_id
            / aggregation_run_id
    ...
    # Phase C inherits the same distinction as Phase B between the logical
    # aggregate-zone ID and its filesystem-safe storage directory. Prefer an
    # exact logical-name directory when present. Otherwise accept exactly one
    # storage-token-equivalent child under an authoritative layout parent.
    wanted = _storage_token(aggregate_zone_id)
    matches: list[Path] = []
    ...
    for parent in preferred_parents:
        ...
        exact = parent / aggregate_zone_id
        if exact.is_dir():
            return exact.resolve()
        ...
        token_matches = [
            ...
            if child.is_dir() and _storage_token(child.name) == wanted
        ]
        if len(token_matches) > 1:
            raise PhaseDDiscoveryError(
                "Phase C zone storage-name resolution is ambiguous for "
                f"{aggregate_zone_id!r} under {parent}: {token_matches}"
            )
        matches.extend(token_matches)

    # Compatibility fallback for older Phase C layouts: search only directories
    # that satisfy the authoritative case/run or case/aggregation/weight lineage
    # constraints, but use the same storage-token equivalence.
    for candidate in run_root.rglob("*"):
        if not candidate.is_dir():
            continue

        if _storage_token(candidate.name) != wanted:
            continue
