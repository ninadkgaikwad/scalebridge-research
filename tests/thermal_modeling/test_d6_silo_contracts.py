...
def test_structurally_uncontrolled_zone_omits_qac_but_keeps_state() -> None:
    no_qac = ZoneSignalAvailability(
        aggregate_zone_id="Corridor",
    ...
    names = [item.name for item in contract.base_columns(independent_zone_id="Corridor")]
    assert "Corridor__zone_temperature" in names
    assert "Corridor__qac" not in names


def test_dependent_contract_supports_mixed_qac_availability() -> None:
    no_qac = ZoneSignalAvailability(
        aggregate_zone_id="Corridor",
        available_disturbances=("qsol1",),
        qac_available=False,
