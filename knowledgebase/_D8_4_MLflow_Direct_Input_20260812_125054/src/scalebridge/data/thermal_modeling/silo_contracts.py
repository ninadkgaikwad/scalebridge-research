...
OPT_BAYES_POLICY_CATALOG: dict[str, PolicyContract] = {
    ...
    "contiguous_identification": PolicyContract(
        ...
        requires_window_id_for_included=True,
        description=(
            "Use one or more deterministic contiguous train/test windows for "
...
class SiloProductContract:
    ...
    def __post_init__(self) -> None:
        ...
        if any(not zone.qac_available for zone in self.current_zones):
            raise D6ContractError(
                "Every current aggregate zone requires QAC as the control input"
            )
